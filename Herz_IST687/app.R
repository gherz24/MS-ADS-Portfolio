...
ui <- fluidPage(
  ...
  #Read the data
  fileInput("upload", label="Test Data Input File", accept = c(".csv")),
...
server <- function(input, output, session) {
  ...
  getTestData <- reactive({
    req(input$upload)
    read_csv(input$upload$expensive)
  })
...
library(caret); library(kernlab); library(e1071);library(tidyverse)
...
use_model_to_predict <- function(df, df_solution){
  ...
  rpart_pred <- predict(rpartmodel,type = "Class")
  #show how the model performed
  confusionMatrix(rpart_pred, test$expensive)
}
